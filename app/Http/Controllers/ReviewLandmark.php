<?php

namespace App\Http\Controllers;

use App\Models\Landmark;
use App\Models\LandmarkReview;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class ReviewLandmark extends Controller
{
    public function AddReviewToLandmark(Request $request, $landmarkid)
    {
        $rules = [
            'rating' => 'required|numeric|min:1|max:5',
            'comment' => 'string|max:1000',
        ];
        $validator = Validator::make($request->all(), $rules);

        if ($validator->fails()) {
            return response()->json([
                'message' => 'Validation failed',
                'errors' => $validator->errors()
            ], 422);
        }
        $review = new LandmarkReview();
        $review->landmark_id = $landmarkid;
        $review->user_id = auth()->id(); // Ensure you have user authentication in place
        $review->rating = $request->input('rating');
        $review->comment = $request->input('comment');
        $review->save();

        return response()->json([
            'message' => 'Review added successfully.',
            'review' => $review
        ], 201);
    }

    // Method to retrieve reviews for a specific city
    public function GetReviewsForLandmark($landmarkid)
    {
        $landmark = Landmark::findOrFail($landmarkid);
        $reviews = $landmark->landmarkreview()->get();

        return response()->json([
            'landmark' => $landmark,
            'reviews' => $reviews
        ], 200);
}
}
